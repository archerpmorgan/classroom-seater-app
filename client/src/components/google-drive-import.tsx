...
import { CloudDownload, RefreshCw, AlertCircle, CheckCircle, FileText, Calendar, FileSpreadsheet, Sheet } from "lucide-react";
...
export default function GoogleDriveImport() {
  ...
  const loadFiles = async () => {
    ...
    try {
      ...
      // Reset selections when loading new files
      ...
      if (data.length === 0) {
        toast({
          ...
          description: "No CSV, Excel, or Google Sheets files found in the specified folder",
        });
      } else {
        toast({
          title: "Files Loaded",
  ...
  const loadSheets = async (file: GoogleDriveFile) => {
    ...
    try {
      const response = await fetch(getApiUrl(`/api/google-drive/sheets/${file.id}`));
      if (!response.ok) {
      ...
      if (data.length === 0) {
        toast({
          ...
          description: "No sheets found in this Google Sheets document",
          variant: "destructive",
        });
      }
    } catch (error) {
      ...
      toast({
        title: "Error",
        description: "Failed to load sheets from Google Sheets document",
        variant: "destructive",
      });
      setSheets([]);
    }
  ...
  const handleFileSelect = (file: GoogleDriveFile) => {
    ...
    // If it's a Google Sheets document, load its sheets
    if (file.mimeType === 'application/vnd.google-apps.spreadsheet') {
      loadSheets(file);
    }
  };

  const importFile = async (fileId: string, fileName: string, sheetName?: string) => {
    try {
      ...
      toast({
        ...
        description: `Imported ${result.count || result.students?.length || 0} students from ${displayName}`,
      });
      ...
      toast({
        title: "Import Failed",
        description: `Failed to import ${displayName}`,
        variant: "destructive",
      });
    }
  };
  ...
  const importMutation = useMutation({
    mutationFn: ({ fileId, fileName, sheetName }: { fileId: string; fileName: string; sheetName?: string }) => 
      importFile(fileId, fileName, sheetName),
  });
    ...
    <Card>
      <CardHeader className="pb-3">
        ...
        <CardDescription className="text-xs leading-relaxed">
          Import CSV/Excel/Sheets files from Drive
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-3 pt-0">
          ...
          <>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1.5 max-h-48 overflow-y-auto">
                {files.map((file) => {
                  const isGoogleSheets = file.mimeType === 'application/vnd.google-apps.spreadsheet';
                  const isExcel = file.mimeType?.includes('spreadsheet') || file.mimeType?.includes('excel');
                  const fileType = isGoogleSheets ? 'Sheets' : (isExcel ? 'XLS' : 'CSV');
                    ...
                    <div key={file.id} className={`border rounded-md p-2 hover:bg-muted/50 space-y-1.5 ${
                      ...
                      <div className="flex items-center gap-1.5">
                        <IconComponent className={`w-3 h-3 flex-shrink-0 ${
                          isGoogleSheets ? 'text-green-600' : (isExcel ? 'text-green-600' : 'text-blue-600')
                        }`} />
                        <Badge variant="secondary" className={`text-xs px-1 py-0 h-4 ${
                          isGoogleSheets ? 'bg-green-100 text-green-700' : (isExcel ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700')
                        }`}>
                          {fileType}
                        </Badge>
                      </div>
                      ...
                      {/* File details and select/import button */}
        ...
        {/* Sheet Selection for Google Sheets */}
        {selectedFile && selectedFile.mimeType === 'application/vnd.google-apps.spreadsheet' && sheets.length > 0 && (
          <>
            <Separator />
            <div className="space-y-2">
              <h3 className="text-xs font-medium text-muted-foreground">
                Select Sheet from "{selectedFile.name}"
              </h3>
              <div className="space-y-2">
                <Select value={selectedSheet} onValueChange={setSelectedSheet}>
                  ...
                  <SelectContent>
                    {sheets.map((sheet) => (
                      <SelectItem key={sheet.sheetId} value={sheet.title} className="text-xs">
                ...
                <Button
                  size="sm"
                  onClick={() => importFile(selectedFile.id, selectedFile.name, selectedSheet)}
